...
function AdminTable({
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      d = d.filter((row) =>
        columns.some((col) =>
          getSafeText(row[col.key]).toLowerCase().includes(q),
        ),
      )
    }
    ...
    if (sortKey) {
      d.sort((a, b) => {
        const va = getSafeText(a[sortKey]).toLowerCase()
        const vb = getSafeText(b[sortKey]).toLowerCase()
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="relative w-full sm:max-w-sm">
          ...
          <input
            ...
            className="w-full h-9 pl-9 pr-3 rounded-xl border border-slate-200 bg-white text-xs text-slate-900 placeholder:text-slate-400 focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 outline-none transition dark:bg-slate-800 dark:border-slate-600 dark:text-slate-100 dark:placeholder:text-slate-500"
          />
        </div>
